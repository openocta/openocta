---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenOcta deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenOcta cron scheduling for periodic checks, or version status checks on a machine running OpenOcta (laptop, workstation, Pi, VPS).
---

# OpenOcta Host Hardening

## Overview

Assess and harden the host running OpenOcta, then align it to a user-defined risk tolerance without breaking access. Use OpenOcta security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpenOcta changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenOcta gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenOcta can access sensitive data.
...
### 2) Run OpenOcta security audits (read-only)

As part of the default read-only checks, run `openocta security audit --deep`. Only offer alternatives if the user requests them:

1. `openocta security audit` (faster, non-probing)
2. `openocta security audit --json` (structured output)

Offer to apply OpenOcta safe defaults (numbered):

1. `openocta security audit --fix`

Be explicit that `--fix` only tightens OpenOcta defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenOcta version/update status (read-only)

As part of the default read-only checks, run `openocta update status`.
...
- Credential hygiene notes (location of OpenOcta creds, prefer disk encryption)
...
- OpenOcta security audit (re-run)
...
After OpenOcta install or first hardening pass, run at least one baseline audit and version check:

- `openocta security audit`
- `openocta security audit --deep`
- `openocta update status`

Ongoing monitoring is recommended. Use the OpenOcta cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `openocta cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `openocta update status`
...
Before creating, `openocta cron list` and match on exact `name`. If found, `openocta cron edit <id> ...`.
If not found, `openocta cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `openocta update status` (preferred for source checkouts and channels)
2. `npm view openocta version` (published npm version)

## OpenOcta command accuracy

Use only supported commands and flags:

- `openocta security audit [--deep] [--fix] [--json]`
- `openocta status` / `openocta status --deep`
- `openocta health --json`
- `openocta update status`
- `openocta cron add|list|runs|run`

Do not invent CLI flags or imply OpenOcta enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenOcta compaction:
